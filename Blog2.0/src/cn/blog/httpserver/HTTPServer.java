...
public class HTTPServer {
    ...
    /**
     * 必须支持的日期的simpledateform兼容格式.请注意，所有生成的日期字段必须仅在RFC 1123格式中，而其他字段则由收件人支持向后兼容.
     */
    ...
    static {
        // 初始化状态描述查找表
        Arrays.fill(statuses, "Unknown Status"); //未知的状态
    ...
    static {
        // 添加一些默认的公共内容类型。
        // 请参阅http://www.iana.org/assignments/media-types/获取完整列表。
    ...
    /**
     * 解码一个InputStream，它的数据有“块”传输编码应用到它，提供基础数据。
     */
    ...
    public static class VirtualHost {
        ...
        /**
         * 设置目录索引文件。对于每个URI以“/”(即一个目录)结尾的请求，索引文件被附加到路径中，如果它存在，所得到的资源将被服务。
         * 如果它不存在，则可以为所请求的目录提供一个自动生成的索引，这取决于是否返回了一个错误。
         *默认目录索引文件是“index.html”。
         *
        ...
        /**
         * 设置是否允许自动生成索引。如果错误，并且请求一个目录资源，那么将返回一个错误。
         *
        ...
        /**
         * 为该服务器添加上下文及其相应的上下文处理程序。路径通过删除拖后的斜杠(根)来规范化。
         *
                ...
                while (!serv.isClosed()) {
                    ...
                    executor.execute(new Runnable() {
                        public void run() {
                            try {
                                try {
                                    sock.setSoTimeout(socketTimeout); //设置超时设置
                                    sock.setTcpNoDelay(true); // 开启缓冲改善延迟
                                    //处理连接
    ...
    public HTTPServer(int port) {
        setPort(port);
        addVirtualHost(new VirtualHost(null)); // 添加默认虚拟主机
    }
    ...
    /**
     * 启动该服务器。如果已经开始，什么都不做。
     * 注意:一旦服务器启动，服务器的配置改变方法及其虚拟主机就不能使用。要修改配置，必须首先停止服务器。
     *
    ...
    public synchronized void start() throws IOException {
        ...
        if (executor == null) // 如果需要，分配默认的执行器。
            executor = Executors.newCachedThreadPool(); // 空闲时不消耗资源
        // 注册所有主机别名(可能已修改)
    ...
    protected void handleConnection(InputStream in, OutputStream out) throws IOException {
        ...
        do {
            ...
            try {
                req = new Request(in);
                handleTransaction(req, resp);//处理连接
    ...
    protected void handleTransaction(Request req, Response resp) throws IOException {
        ...
        if (preprocessTransaction(req, resp))//预处理的事务
            handleMethod(req, resp);//处理方法
    }
